...
PeerImp::send(std::shared_ptr<Message> const& m)
{
    ...
    if (messageType == 33)
    {
       if (!validator)
       {
            ...
            if (_grpcOut == 2)
                std::cout << "Message not sent, validation not originated here" << std::endl;
    ...
    try
    {
      ...
      auto key = sha512Half(makeSlice(m->validation()));
            ...
            !added)
        {
            ...
            std::cout << currentDateTime() << " | Validation Duplicated | " << key << std:: endl;
            return;
        }
        std::cout << currentDateTime() << " | Validation Unique | " << key << std:: endl;
